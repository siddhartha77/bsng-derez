...
static OSErr ProcessItem(FSSpecPtr myFSSPtr)
{
	...
	CIconResource*  cicn;
	Str255          version;
	Str255          headerFilename;
	Str255          rezFilename;
	
	/* Only process applications */
	if (!(GetFileType(myFSSPtr) == 'APPL')) {
	    ErrorAlert(kErrStringListID, kNotAPPLErr, kErrStringListID + kNotAPPLErr);
	    
	    return (err);
	}
	
	GetIndString(headerFilename, kOutputFilenamesStringListID, kOutputFilenameHeader);
    GetIndString(rezFilename, kOutputFilenamesStringListID, kOutputFilenameRez);

	oldResRef = CurResFile();

	resRef = FSpOpenResFile(myFSSPtr, fsRdPerm);
	if (resRef == NULL) {
        ErrorAlert(kErrStringListID, kResourceForkErr, kErrStringListID + kResourceForkErr);
        return(err);
    }
	...
	// Get icon data
    cicn = MakeCICN(myFSSPtr);
    if (cicn == NULL) {
        ErrorAlert(kErrStringListID, kCreatecicnErr, kErrStringListID + kCreatecicnErr);
	...
	// Now spit out data
	err = GenerateHeader(headerFilename, myFSSPtr, version);
	err = GenerateRez(rezFilename, headerFilename, myFSSPtr, cicn);
